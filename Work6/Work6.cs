...
namespace Work6
{
    ...
    static class Work6
    {
        ...
        private static void StringWorks()
        {
            ...
            var (lowerCount, upperCount) = CalculateCase(text);

            Console.WriteLine($"\nКоличество строчных  букв: {lowerCount}\n" +
                                $"Количество прописных букв: {upperCount}");
        }

        private static (int lowerCount, int upperCount) CalculateCase(string text)
        {
            var lowerCount = 0;
            var upperCount = 0;
        ...
        static void Main()
        {
            while (true)
            {
                ...
                switch (item)
                {
                    ...
                    case 5: StringWorks();       break;
                    case 6: ConfirmationExit();  break;
